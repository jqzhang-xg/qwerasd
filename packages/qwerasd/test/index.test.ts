import { useQwerty, table, DetectResult } from "../src/index";
...
describe("鍵盤連續字檢測器", () => {
  ...
  describe("QWERTY 鍵盤檢測", () => {
    let detector: ReturnType<typeof useQwerty>;

    beforeEach(() => {
      detector = useQwerty(3);
    });
  ...
  describe("轉型調用", () => {
    ...
    test("toString() 應該返回總結果的字符串", () => {
      const trueDetector = useQwerty(3);
      const falseDetector = useQwerty(3);
  ...
  describe("table 導出", () => {
    test("應該導出正確的鍵盤配置表", () => {
      expect(table).toHaveProperty("qwerty");
      expect(table).toHaveProperty("uppercase");
      expect(table).toHaveProperty("lowercase");
      expect(table).toHaveProperty("number");

      expect(table.qwerty).toEqual(["qwertyuiop", "asdfghjkl", "zxcvbnm"]);
      expect(table.number).toEqual(["0123456789"]);
      expect(table.uppercase).toEqual(["ABCDEFGHIJKLMNOPQRSTUVWXYZ"]);
      expect(table.lowercase).toEqual(["abcdefghijklmnopqrstuvwxyz"]);
    });
  });
});
